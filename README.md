...
`llms.txt` (at the repo root) is a single-file reference that documents every public API, configuration key, and template hook in this package — formatted for LLM consumption.

When working with Claude Code, Cursor, Copilot, or any other coding agent, point it at `llms.txt` so it has accurate, version-matched context for the install you're using:

```text
# In your project, fetch the matching version into the agent's context:
curl -O https://raw.githubusercontent.com/cuongnbms/django-admin-extended/v6.0.0/llms.txt
```
